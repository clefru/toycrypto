...
class EC(base.Group):
  ...
  def fromX(self, x):
    y = self.field.plus(x.scalarPow(3), self.field.plus(self.field.mul(self.A, x), self.B)).sqrt()
    if not y:
      return None
  ...
  def plus(self, a, b):
    ...
    if a.x == b.x and a.y == b.y:
      if a.y == f.plusID():
        ...
        vu = f.mul(f.plus(minus_x3,
                          f.plus(f.mul(self.A, a.x),
      ...
      if a.x == b.x:
        ...
        lmbd = f.mul(
          f.plus(b.y, a.y.plusInv()),
          f.plus(b.x, a.x.plusInv()).mulInv())
        vu = f.mul(f.plus(f.mul(a.y, b.x), f.mul(b.y, a.x).plusInv()),
                           f.plus(b.x, a.x.plusInv()).mulInv())
    lmbd2 = f.mul(lmbd, lmbd)
    return self.Element(self,
      f.plus(lmbd2, f.plus(a.x, b.x).plusInv()),
      f.plus(
        f.mul(
          lmbd,
          f.plus(a.x, b.x)),
  ...
  class Element(base.Group.Element):
    ...
    def __eq__(self, a):
      return type(self) == type(a) and self.field == a.field and self.x == a.x and self.y == a.y
...
class ECSubfield(base.Group):
  ...
  class Element(base.Group.Element):
    ...
    def __eq__(self, a):
      return type(self) == type(a) and self.field == a.field and self.point == a.point
